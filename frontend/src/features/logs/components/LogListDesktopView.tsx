      ...
      {services.length > 0 && (
        <div className="flex items-center justify-between gap-3 mb-5 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="px-3 py-1.5 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-sm font-bold">
              {t('logs.filter.all')} {services.length}
            </span>
            <span className="px-3 py-1.5 rounded-full border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-sm font-bold">
              {t('logs.filter.error')} {errorServiceCount}
            </span>
            <span className="px-3 py-1.5 rounded-full border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 text-sm font-bold">
              {t('logs.filter.warn')} {warnServiceCount}
            </span>
            <span className="px-3 py-1.5 rounded-full border border-slate-200 dark:border-ui-border-dark bg-white dark:bg-bg-surface-dark text-slate-500 dark:text-text-muted-dark text-sm font-bold">
...
function LogServiceTable({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 dark:border-ui-border-dark bg-white dark:bg-bg-surface-dark">
      <div className="grid grid-cols-[minmax(220px,1.3fr)_112px_120px_minmax(220px,1.4fr)_150px_92px] gap-3 px-4 py-2.5 bg-slate-50/80 dark:bg-ui-hover-dark/40 border-b border-slate-100 dark:border-ui-border-dark text-sm font-bold uppercase tracking-wide text-slate-400 dark:text-text-dim-dark">
        ...
        return (
          ...
          >
            <div className="min-w-0">
              ...
              <div className="text-sm text-slate-400 dark:text-text-dim-dark truncate">{service.id}</div>
            </div>
            <div className="flex flex-col justify-center text-sm text-slate-500 dark:text-text-muted-dark">
              ...
              <span className="text-sm">{logs.length} recent</span>
            </div>
            ...
            <div className="min-w-0 flex items-center gap-2 font-mono text-sm">
              {latest ? (
                <>
                  <span
