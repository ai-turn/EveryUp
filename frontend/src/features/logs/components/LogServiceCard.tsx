      ...
      <div className="flex gap-2">
        {levelsOrder.map((level) => (
          <div key={level} className="flex-1 flex flex-col items-center gap-0.5">
            ...
            <span className={`text-sm font-black tabular-nums ${level === 'error' && levelCounts[level] > 0 ? 'text-red-500' : level === 'warn' && levelCounts[level] > 0 ? 'text-amber-500' : 'text-slate-700 dark:text-text-base-dark'}`}>
              {levelCounts[level]}
            </span>
          </div>
        ))}
