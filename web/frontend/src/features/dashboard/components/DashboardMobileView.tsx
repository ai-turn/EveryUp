...
export function DashboardMobileView() {
  ...
  const [agentServiceCount, setAgentServiceCount] = useState(0);
  ...
  useEffect(() => {
    ...
    async function fetchAgents() {
      try {
        ...
        const serviceLists = await Promise.all(agents.map(agent => api.getAgentServices(agent.id)));
        if (!alive) return;
        setAgentCount(agents.length);
        setAgentServiceCount(serviceLists.reduce((sum, services) => sum + services.length, 0));
      } catch {
        if (!alive) return;
        setAgentCount(0);
        setAgentServiceCount(0);
      }
    }
    fetchAgents();
    return () => {
    ...
    <div className="space-y-4">
      ...
      <section>
        <div className="flex items-center justify-between p-4 pb-3">
          <div className="flex items-center gap-2">
            <IconInfra size={18} className="text-primary" />
            <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Connected Agents</h2>
          </div>
          <span className="text-sm font-medium text-slate-500 dark:text-text-muted-dark">{agentCount}</span>
        </div>
        <div className="px-3 pb-3">
          <div className="rounded-lg bg-slate-50 dark:bg-ui-hover-dark p-3">
            <p className="text-sm text-slate-500 dark:text-text-muted-dark">Agent-discovered services</p>
            <p className="mt-1 text-xl font-semibold text-slate-900 dark:text-white">{agentServiceCount}</p>
          </div>
        </div>
        <div className="mx-4 h-px bg-slate-200 dark:bg-ui-border-dark" />
      </section>
