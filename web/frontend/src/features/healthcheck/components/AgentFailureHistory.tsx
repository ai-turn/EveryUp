...
export function AgentFailureHistory({ agentId, serviceKey, refreshKey }: AgentFailureHistoryProps) {
  ...
  if (loading) {
    return (
      <div className="mb-8 p-6 rounded-xl border border-slate-200 dark:border-ui-border-dark bg-white dark:bg-chart-bg animate-pulse">
        <div className="h-5 bg-slate-200 dark:bg-ui-active-dark rounded w-40 mb-4" />
        {[1, 2].map((i) => (
          <div key={i} className="h-14 bg-slate-100 dark:bg-ui-hover-dark rounded-lg mb-3" />
        ))}
      </div>
    );
  }
    ...
    <div className="mb-8 p-6 rounded-xl border border-slate-200 dark:border-ui-border-dark bg-white dark:bg-bg-surface-dark">
      ...
      {events.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 gap-3">
          ...
          <p className="text-slate-500 dark:text-text-muted-dark text-sm">{t('장애 기록이 없습니다')}</p>
        </div>
      ) : (
