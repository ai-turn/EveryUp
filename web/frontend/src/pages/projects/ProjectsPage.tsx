    ...
    <article className="rounded-xl border border-ui-border bg-bg-surface p-5">
      ...
      {(unassignedAgents.length > 0 || unassignedMonitors.length > 0 || unassignedDirect.length > 0 || unassignedInfrastructure.length > 0) && (
        <div className="mt-5 space-y-2 border-t border-ui-border pt-4">
          {unassignedAgents.length > 0 && <div className="flex gap-2"><Select value={agentId} onChange={event => setAgentId(event.target.value)} aria-label={t('Docker 환경 추가')}><option value="">{t('Docker 환경 추가')}</option>{unassignedAgents.map(agent => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</Select><Button variant="secondary" disabled={!agentId} onClick={() => { onAssignAgent(agentId); setAgentId(''); }}>{t('배정')}</Button></div>}
...
export function ProjectsPage() {
  ...
  return <div>
    <PageHeader title={t('Projects')} subtitle={t('Docker 환경, 업타임 모니터, 직접 Observed Service와 Collector를 논리적으로 묶습니다.')}><Button onClick={() => setEditing(null)}><MaterialIcon name="add" />{t('Project 추가')}</Button></PageHeader>
    ...
    <ConfirmDialog isOpen={Boolean(deleting)} onClose={() => setDeleting(null)} onConfirm={() => void deleteProject()} title={t('Project를 삭제할까요?')} message={t('배정된 Docker 환경, 업타임 모니터, 직접 서비스와 Collector는 삭제되지 않고 미분류로 남습니다.')} confirmLabel={t('삭제')} isProcessing={deletingProject} />
  </div>;
}
