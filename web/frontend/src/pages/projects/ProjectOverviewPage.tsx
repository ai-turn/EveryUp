  ...
  return (
    <div className="space-y-5">
      ...
      <section aria-label="Project 요약" className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-ui-border bg-bg-surface p-4"><p className="text-xs text-text-muted">수집 확인 필요</p><p className={`mt-1 font-mono text-2xl font-bold tabular-nums ${connectionIssues ? 'text-status-warn' : 'text-status-healthy'}`}>{connectionIssues}</p><p className={`mt-3 text-xs ${connectionIssues ? 'text-status-warn' : 'text-status-healthy'}`}>{connectionIssues ? '지연 또는 미확인 대상이 있습니다' : '연결이 모두 최신입니다'}</p></div>
        <div className="rounded-xl border border-ui-border bg-bg-surface p-4"><p className="text-xs text-text-muted">업타임 장애</p><p className={`mt-1 font-mono text-2xl font-bold tabular-nums ${unhealthyMonitors ? 'text-status-error' : 'text-status-healthy'}`}>{unhealthyMonitors}</p><p className={`mt-3 text-xs ${unhealthyMonitors ? 'text-status-error' : 'text-status-healthy'}`}>{unhealthyMonitors ? '즉시 확인이 필요한 모니터가 있습니다' : '현재 업타임 장애가 없습니다'}</p></div>
      </section>
