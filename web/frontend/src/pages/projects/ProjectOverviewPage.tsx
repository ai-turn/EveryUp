  ...
  return (
    <div className="space-y-5">
      ...
      <section aria-label="Project 요약" className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-ui-border bg-bg-surface p-4"><p className="text-xs text-text-muted">모니터링 대상</p><p className="mt-1 font-mono text-2xl font-bold tabular-nums text-text-base">{totalTargets}</p><p className="mt-3 text-xs font-semibold text-text-muted">환경, 모니터, 직접 연결, 인프라</p></div>
        <div className="rounded-xl border border-ui-border bg-bg-surface p-4"><p className="text-xs text-text-muted">수집 확인 필요</p><p className={`mt-1 font-mono text-2xl font-bold tabular-nums ${connectionIssues ? 'text-status-warn' : 'text-status-healthy'}`}>{connectionIssues}</p><p className={`mt-3 text-xs font-semibold ${connectionIssues ? 'text-status-warn' : 'text-status-healthy'}`}>{connectionIssues ? '지연 또는 미확인 대상이 있습니다' : '연결이 모두 최신입니다'}</p></div>
        <div className="rounded-xl border border-ui-border bg-bg-surface p-4"><p className="text-xs text-text-muted">업타임 장애</p><p className={`mt-1 font-mono text-2xl font-bold tabular-nums ${unhealthyMonitors ? 'text-status-error' : 'text-status-healthy'}`}>{unhealthyMonitors}</p><p className={`mt-3 text-xs font-semibold ${unhealthyMonitors ? 'text-status-error' : 'text-status-healthy'}`}>{unhealthyMonitors ? '즉시 확인이 필요한 모니터가 있습니다' : '현재 업타임 장애가 없습니다'}</p></div>
      </section>
      ...
      {totalTargets === 0 ? <section className="rounded-xl border border-ui-border bg-bg-surface"><EmptyState icon="folder_open" title="아직 배정된 대상이 없습니다" description="Project 관리 화면에서 환경과 모니터링 대상을 배정하세요." action={{ label: 'Project 관리', onClick: () => navigate('/projects') }} /></section> : (
        <section className="rounded-xl border border-ui-border bg-bg-surface">
          <div className="border-b border-ui-border px-4 py-3.5"><h2 className="text-base font-bold text-text-base">대상</h2><p className="mt-0.5 text-sm text-text-muted">서비스 상태와 수집 상태는 대상 상세에서 분리해 확인할 수 있습니다.</p></div>
