...
export function DirectMetricsDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!serviceRow.signals.includes('metrics')) {
        setError(t('이 서비스에는 Metrics 기능이 연결되어 있지 않습니다.'));
        return;
      }
  ...
  const runConfirmedAction = async () => {
    ...
    try {
      if (confirmAction === 'rotate') {
        ...
        toast.success(t('직접 Metrics 서비스를 삭제했습니다.'));
  ...
  if (error || !service) {
    return <EmptyState icon="error_outline" title={t('직접 Metrics 서비스를 불러오지 못했습니다')} description={error ?? undefined} action={{ label: t('Metrics로 돌아가기'), onClick: () => navigate('/metrics') }} />;
  }
  ...
  const confirmCopy = {
    ...
    delete: {
      ...
      message: t('이 서비스의 메트릭과 연결된 다른 신호 데이터, 직접 수집 연결, 대상별 알림 규칙을 함께 삭제합니다.'),
      label: t('삭제'),
      variant: 'danger' as const,
    },
  };
