...
| 항목 | 건수 | 조치 |
|------|------|------|
| 시맨틱 토큰 + `dark:` 짝 | 17 | `dark:` 제거 |
| `*-dark` 접미 토큰 직접 사용 | 54 | 사이트별 확인 후 치환 |
| `bg-white` / `slate-*` 하드코딩 | 126 | 상태색 목적이 아니면 토큰으로 |

**대부분은 색 문제가 아니라 다른 문제의 증상이다.** 2026-07-27에 225건을 분류해보니 세 덩어리가 각각 하나의 원인에서 나왔다 — 순수 중복 5건(토큰이 이미 전환하는데 `dark:` 짝을 덧붙임), `FormStep`/`Field` 중복 정의 6건, 이름 없는 "떠오른 표면" 역할 8건. 남은 것도 먼저 **역할에 이름이 없는지, 컴포넌트가 중복인지**를 의심할 것. 색을 치환하면 증상만 가려진다.
