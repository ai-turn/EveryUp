...
This Compose file runs Web only. Run `agent/docker-compose.yml` on each Docker
...
| `EVERYUP_ENCRYPTION_KEY` | auto-generated | 64-char hex (32 bytes) AES key for secrets (Docker collector API keys, notification channels). **Set this in production** so secrets survive a DB restore; otherwise a key is generated and stored in the DB |
